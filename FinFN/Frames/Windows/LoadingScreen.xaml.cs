        ...
        private async Task FetchVersion(string endpoint)
        {
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    {
                        MessageBox.Show("Launcherisi versio on liian vanha. Lataa uusin versio discordistamme!", "FinFN");
            ...
            {
                MessageBox.Show("Serverit on todennäköisesti alhaalla, yritä myöhemmin uudelleen!", "FinFN");
                Application.Current.Shutdown();
            }
        }
    }
